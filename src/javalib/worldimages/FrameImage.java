...
/**
 * <p>
 * The class to represent rectangular outline images drawn by the world when
 ...
 */
public class FrameImage extends WorldImage {
    public int width;
    public int height;
     ...
     */
    public FrameImage(Posn pinhole, int width, int height, Color color) {
        super(pinhole, color);
        this.width = width;
        this.height = height;
    }
    ...
    public FrameImage(Posn pinhole, int width, int height, IColor color) {
        super(pinhole, color);
        this.width = width;
        this.height = height;
    }
    ...
    public void draw(Graphics2D g) {
        ...
        g.setPaint(color);
        // draw the object
        g.draw(new Rectangle2D.Double(this.pinhole.x - this.width / 2,
    ...
    /**
     * Produce the frame with the pinhole moved by the given (dx, dy)
     * 
     * @param dx
    ...
    public WorldImage getMovedImage(int dx, int dy) {
        return new FrameImage(
                new Posn(this.pinhole.x + dx, this.pinhole.y + dy), this.width,
                this.height, this.color);
    }

    /**
     * Produce the frame with the pinhole moved to the given location
     * 
     * @param p
     *            the given location
     */
    public WorldImage getMovedTo(Posn p) {
        return new FrameImage(p, this.width, this.height, this.color);
    }
