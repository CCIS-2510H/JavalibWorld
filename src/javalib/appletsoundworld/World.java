...
/**
 * World for programming interactive games - with graphics, key events, timer,
 * and sound effects.
 * 
 * @author Viera K. Proulx, Virag Shah
 * @since October 29, 2009, October 21 2010, July 12 2011, February 22 2012, 15
 *        November 2012
 */
abstract public class World implements SoundConstants {
    ...
    private transient ArrayList<String> pressedKeys = new ArrayList<String>();
    ...
    protected HashMap<String, TuneBucket> keyReleasedTunes = new HashMap<String, TuneBucket>();
    ...
    // ///////////////////////////////////////////////////////////////////////
    /**
     * Initialize the world with the canvas, its key adapter, and mouse adapter.
     */
    ...
    void initMusic() {
        ...
        if (javalib.tunes.MusicBox.SYNTH_READY) {
            ...
            System.out
                    .println("MIDI synthesizer or the soundbank not available.");
    ...
    /**
     * <p>
     * Change the program for the {link MusicBox MusicBox} to the given list of
     * instruments.
     * </p>
     * <p>
     * If there are fewer than 16 instruments given, all the remaining ones are
     * set to play piano.
     * </p>
     * 
     * @param myInstruments
     *            an <code>ArrayList</code> of up to 16 instrument numbers, --
    ...
    public void programChange(ArrayList<Integer> myInstruments) {
        ...
        // if the synthesizer is running, notify the MusicBox of the program
        // change
        if (javalib.tunes.MusicBox.SYNTH_READY) {
    ...
    private void recordProgramChange() {
        ...
        for (String key : this.keyReleasedTunes.keySet())
            this.keyReleasedTunes.get(key)
                    .recordProgramChange(this.instruments);
    }
    ...
    /**
     * Pause at the start when building GUIs and installing listeners and at the
     * end so the last tunes are heard
     */
     ...
     * 
     * @param w
     *            the width of the <code>Canvas</code>
     * @param h
     *            the height of the <code>Canvas</code>
     * @param speed
    ...
    protected void processTick() {
        try {
            ...
            // thus currentTickTunes would never know that one beat note even
            // started playing
            this.tickTunes.playTunes();
            ...
            /*
             * else // the world has stopped - clear the tick tune and key tunes
             * buckets this.stopWorld();
             */
        }

        catch (RuntimeException re) {
            re.printStackTrace();
            this.lastWorld.lastImage.overlayImages(new TextImage(new Posn(40,
                    40), re.getMessage(), Color.red));
            this.stopWorld();
            // throw re;
    ...
    /**
     * <P>
     * User defined method to be invoked by the timer on each key event. Update
     * the <code>{@link World World}</code>.
     * </P>
     * <P>
    ...
    /**
     * The method invoked by the key adapter on key pressed event. Clears the
     * contents of the keyTunes bucket. Delegates to the user to define a new
     * state of the world, then plays the tunes deposited in the keyTunes
     * bucket. It then copies these tunes to the keyReleasedTunes hashmap, using
     * the key pressed as the hashmap key.
     */
        ...
        try {
            if (this.worldExists && !this.pressedKeys.contains(ke)) {
                ...
                if (this.worldExists) {
                    ...
                    // save what is currently playing so it plays for the
                    // desired
                    // number of ticks
    ...
    /**
     * The method invoked by the key adapter when key is released. Delegates to
     * the user to define a new state of the world, then removes from the
     * KeyReleasedTunes bucket those tunes that started playing when the given
     * key was pressed.
     * 
     */
    ...
    /**
     * The method checks if the given TuneBucket is still being played. Used to
     * incorporate the scenario where two different keys are playing the same
     * tune on the same channel and if one key is released the tune should still
     * continue to play until the second key is released.
     * 
     * @param tuneBucket
     *            the TuneBucket to check for
    ...
    /**
     * <P>
     * User defined method to be invoked by the key adapter when a key is
     ...
     * </P>
     * <P>
     * Implement this method in the game world class
     * </P>
     */
    ...
    public void testOffKey(String ke) {
        ...
        if (tmp != null) {
            /**
             * Remove only the tunes/notes associated with the corresponding key
             * released.
             */
            for (int i = 0; i < 16; i++) {
                ArrayList<Note> notes = tmp.tunes.get(i).getChord().notes;
                for (Note n : notes)
                    this.currentKeyTunes.tunes.get(i).getChord().notes
                            .remove(n);
            }
            tmp.clearBucket();
        }
...
class MyKeyAdapter extends KeyAdapter {
    ...
    protected void displayInfo(KeyEvent e, String s) {
        ...
        if (// id == KeyEvent.KEY_TYPED ||
        id == KeyEvent.KEY_PRESSED || id == KeyEvent.KEY_RELEASED) {
            ...
            /*
             * else{ keyString = KeyEvent.getKeyText(e.getKeyCode()); }
             */
        }

        /*
         * if (id == KeyEvent.KEY_RELEASED) System.out.println("Key " +
         * keyString + " keyCode " //+ e.getKeyCode() + " event: " +
         * "released");
         */
        /*
         * if (id == KeyEvent.KEY_TYPED) System.out.println("Key " + keyString +
         * " keyCode " + e.getKeyCode() + " event: " + "typed");
         */
        /*
         * if (id == KeyEvent.KEY_PRESSED) System.out.println("Key " + keyString
         * + " keyCode " //+ e.getKeyCode() + " event: " + "pressed");
         */
