 ...
 * 
 * <p>
 * An example of the use of the drawing methods in the 'worldimages' library,
 * augmented to illustrate the behavior of the 'soundworld':
 * </p>
 * 
 * <p>
 * On tick the face changes every four ticks from happy to sad, one note plays
 * for four ticks, another plays for the first two ticks only the instrument
 * changes from happy to sad (piano to tube)
 * 
 * <p>
 * key events can play bird-tweet on "up" key, change state with space bar,
 * force end of the world on "x", induce tick-controlled world ending through
 * "q" key (by setting the condition that is evaluated on each tick)
 * </p>
 * 
...
class DrawFace extends World {
    ...
    WorldImage leftEyeImage = new DiskImage(new Posn(35, 35), 5, new Blue())
            .overlayImages(new DiskImage(new Posn(37, 35), 2, new White()));

    // produce the image of a right blue eye with a white pupil
    WorldImage rightEyeImage = new DiskImage(new Posn(65, 35), 5, new Blue())
            .overlayImages(new DiskImage(new Posn(67, 35), 2, new White()));
    ...
    WorldImage happyMouthImage = new DiskImage(new Posn(50, 60), 20, new Red())
            .overlayImages(new DiskImage(new Posn(50, 50), 20, new Yellow()));

    // produce an image of a sad red mouth at location 50, 60
    // assume it will be overlaid with the image of the face
    // that is a yellow disk of radius 40
    WorldImage sadMouthImage = new DiskImage(new Posn(50, 60), 20, new Red())
            .overlayImages(new DiskImage(new Posn(50, 70), 20, new Yellow()));

    // produce an image of the nose as black triangle
    WorldImage noseImage = new TriangleImage(new Posn(50, 40),
            new Posn(40, 60), new Posn(50, 60), new Black());

    // produce the image of a happy face on a green background
    WorldImage happyFaceImage = new RectangleImage(new Posn(50, 50), 100, 100,
            new Green()).overlayImages(new DiskImage(new Posn(50, 50), 40,
            new Yellow()), this.happyMouthImage, this.noseImage,
            this.leftEyeImage, this.rightEyeImage);

    // draw the face on a green background on the given Canvas
    WorldImage sadFaceImage = new RectangleImage(new Posn(50, 50), 100, 100,
            new Green()).overlayImages(new DiskImage(new Posn(50, 50), 40,
            new Yellow()), this.sadMouthImage, this.noseImage,
            this.leftEyeImage, this.rightEyeImage);
...
class ExamplesDrawFace {
    ...
    void testOnTick(Tester t) {
        ...
        t.checkExpect(
                nowPlaying.contains(this.dfw.sadInstrument, new Note("C4n4")),
                true);
        t.checkExpect(
                nowPlaying.contains(this.dfw.sadInstrument, new Note("E4n2")),
                true);
        ...
        t.checkExpect(nowPlaying.bucketSize(), 2);
        t.checkExpect(
                nowPlaying.contains(this.dfw.sadInstrument, new Note("C4n3")),
                true);
        t.checkExpect(
                nowPlaying.contains(this.dfw.sadInstrument, new Note("E4n1")),
                true);
        System.out.println("Now playing on tick count " + this.dfw.tickCount
                + "\n" + nowPlaying.toIndentedString(" "));
        ...
        t.checkExpect(
                nowPlaying.contains(this.dfw.sadInstrument, new Note("C4n2")),
                true);
        t.checkExpect(
                nowPlaying.contains(this.dfw.sadInstrument, new Note("E4n2")),
                false);
        ...
        t.checkExpect(nowPlaying.bucketSize(), 1);
        t.checkExpect(
                nowPlaying.contains(this.dfw.sadInstrument, new Note("C4n1")),
                true);
        t.checkExpect(
                nowPlaying.contains(this.dfw.sadInstrument, new Note("E4n2")),
                false);
        ...
        t.checkExpect(nowPlaying.bucketSize(), 2);
        t.checkExpect(
                nowPlaying.contains(this.dfw.happyInstrument, new Note("C4n4")),
                true);
        t.checkExpect(
                nowPlaying.contains(this.dfw.happyInstrument, new Note("E4n2")),
                true);
        System.out.println("Now playing on tick count " + this.dfw.tickCount
                + "\n" + nowPlaying.toIndentedString(" "));
    ...
    void testOnKeyEvent(Tester t) {
        ...
        t.checkExpect(this.dfw.lastWorld,
                new WorldEnd(true, this.dfw.makeImage()));
