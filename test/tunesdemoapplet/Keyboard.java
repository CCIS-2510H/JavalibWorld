...
/**
 * A comprehensive demo of the javalib.worldimages and javalib.tunes package: An
 * implementation of a keyboard demo. Two rows of the computer keyboard are
 * designed to play the white keys of the piano keyboard.
 * 
...
public class Keyboard {
    ...
    protected ArrayList<String> keystrings = new ArrayList<String>(
            Arrays.asList("a", "s", "d", "f", "g", "h", "j", "k", "z", "x",
                    "c", "v", "b", "n", "m", ","));

    /** the list of notes played for the two rows of piano keyboard keys */
    protected ArrayList<String> keylabels = new ArrayList<String>(
            Arrays.asList("C", "D", "E", "F", "G", "A", "B", "C", "C", "D",
                    "E", "F", "G", "A", "B", "C"));

    /** the list of pitches for the two rows of piano keyboard keys */
    protected ArrayList<Integer> pitchlist = new ArrayList<Integer>(
    ...
    /**
     * the constructor: initializes the keyboard key positions, labels, note
     * names to show, notes to play, and positions of (inactive) black keys
     */
    public Keyboard() {
        this.initKeyboard();
    }
    ...
    protected void initKeyboard() {
        ...
        for (int i = 0; i < octave; i++) {
            this.keys.add(new Key(new Posn(x + i * 23, y), this.keystrings
                    .get(i), this.keylabels.get(i), this.pitchlist.get(i)));
        }
        ...
        for (int i = 0; i < octave; i++) {
            this.keys.add(new Key(new Posn(x + i * 23, y), this.keystrings
                    .get(i + octave), this.keylabels.get(i + octave),
    ...
    /**
     * Produce the note to be played in response to the given key event. Assumes
     * a valid key event
     * 
     * @param ke
    ...
    /**
     * Stop playing the note activated by the given key press. Ignore if the key
    ...
    /**
     * <p>
     * Produce the image of the keyboard panel:
     * </p>
     * <p>
     * draw the header and the instructions for playing
     * </p>
     * <p>
     * draw the white keys on this keyboard
     * </p>
     * <p>
     * draw the (inactive) black keys
     * </p>
     */
    public WorldImage keyboardImage() {
        ...
        return new TextImage(new Posn(25 + label.length() * 3, 26), label,
                Color.red).overlayImages(
                new TextImage(new Posn(25 + rules.length() * 3, 41), rules,
                        Color.blue), new TextImage(new Posn(20 + 100, 61),
                        "4th octave", Color.red), new TextImage(new Posn(
                        20 + 297, 61), "5th octave", Color.red), this
                        .allKeyImages());
    }
    ...
    protected WorldImage allKeyImages() {
        ...
        for (Posn pos : this.blackKeysCtr)
            base = base.overlayImages(new RectangleImage(pos, 10, 24,
                    Color.black));
...
/**
 * A class that represents one key on a piano keyboard. It records its position
 * in the Canvas, the pitch it plays, the key that activates it, and the color
 * (that changes when the key is playing).
 * 
 * @author Viera K. Proulx
 * @since 15 August 2012
 * 
...
class Key {
    ...
    /**
     * the position of the center of the label that shows what note this piano
     * key plays
     */
    protected Posn labelPosCtr;

    /**
     * the position of the enter of the String that shows which key event
     * activated this piano key
     */
     ...
     * 
     * @param pos
     *            the position of the NW corner of this piano key
     * @param ke
     *            the key event that plays this piano key
     * @param label
     *            the note name of the note this piano key plays
     * @param pitch
     *            the pitch of the note this piano key plays
     */
    ...
    public WorldImage keyImage() {
        return new RectangleImage(this.posCtr, 23, 59, Color.black)
                .overlayImages(new RectangleImage(this.posCtr, 21, 57,
                        Color.lightGray), new TextImage(this.labelPosCtr,
                        this.label, this.col), new TextImage(this.kePosCtr,
                        this.ke, Color.darkGray));
    }
