...
public class ExamplesWorld extends World {
    ...
    ExamplesWorld(Posn pos, int height) {
        this(pos, false, height);
    }
    ...
    public World onKeyEvent(String ke) {
        ...
        if (ke.equals("left"))
            return new ExamplesWorld(new Posn(this.pos.x - 20, this.pos.y),
                    this.width);
        if (ke.equals("up"))
            return new ExamplesWorld(new Posn(0, this.pos.y), this.width);
        if (ke.equals("E"))
            return new ExamplesWorld(this.pos, true, this.width);
        if (ke.equals("q"))
    ...
    public World onTick() {
        System.out.println("Tick -- pos = (" + this.pos.x + ", " + this.pos.y
                + ")");
        return new ExamplesWorld(new Posn((this.pos.x + 2) % this.width,
    ...
    public World onMouseClicked(Posn loc) {
        ...
        return new ExamplesWorld(new Posn(loc.x, loc.y), this.width);
    }
