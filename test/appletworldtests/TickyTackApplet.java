...
/**
 * Applet wrapper for the TickyTack world
 * 
 * @author Viera K. Proulx
 * @since March 7 2012
 */
public class TickyTackApplet extends WorldApplet {
    private static final long serialVersionUID = 1859987894859996363L;

    /** Produce a new TickyTackWorld */
    public World getNewWorld() {
        Cloud cloud = new Cloud(new Posn(200, 100), 90, 60);
        Sun sun = new Sun(25);

        return new TickyTack(cloud, sun);
    }
    ...
    /** Set the speed to be four times the normal, so the cloud moves faster */
...
class ExamplesTickyTack {
    ...
    AppletCanvas c = new AppletCanvas(600, 300);

    void showWorld() {
        c.setVisible(true);
        c.drawImage(this.wholeworld);
    }

    TickyTack tworld = new TickyTack(this.cloud, new Sun(25));

    void reset() {
        this.sun = new Sun(25);
        this.cloud = new Cloud(new Posn(200, 100), 90, 60);
        this.tworld = new TickyTack(this.cloud, new Sun(25));
    }
    ...
    void testOnTick(Tester t) {
        this.reset();
        this.tworld.onTick();
        t.checkExpect(this.tworld, new TickyTack(new Cloud(new Posn(204, 100),
                90, 60), this.sun));
    }
    ...
    void testOnKeyEvent(Tester t) {
        ...
        t.checkExpect(this.tworld, new TickyTack(this.cloud, new Sun(28)));
        TickyTack ttw = new TickyTack(this.cloud, new Sun(28));
        ttw.onKeyEvent(" ");
        t.checkExpect(ttw, new TickyTack(this.cloud, new Sun(1)));
    ...
    void testOnMouseClicked(Tester t) {
        ...
        t.checkExpect(this.tworld, new TickyTack(new Cloud(new Posn(150, 80),
                90, 60), this.sun));
