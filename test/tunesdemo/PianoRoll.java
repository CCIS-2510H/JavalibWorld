...
/**
 * A world that represents three melodies to be played by the demo program. It
 * provides the <code>nextChord</code> method that traverses the chords in a
 * circular manner. (In effect it is a circular iterator.)
 */
public class PianoRoll implements SoundConstants {
    ...
    protected ArrayList<ArrayList<Note>> mytunes = new ArrayList<ArrayList<Note>>();

    /**
     * Frere Jacques melody !! the note DownG is below the staff lines: not
     * shown but played.
     */
    protected static ArrayList<Note> mytune1 = new Chord(0, 0, 0, 0, noteC, 0,
            noteD, 0, noteE, 0, noteC, 0, noteC, 0, noteD, 0, noteE, 0, noteC,
            0, noteE, 0, noteF, 0, noteG, 0, 0, 0, noteE, 0, noteF, 0, noteG,
            0, 0, 0, noteG, noteA, noteG, noteF, noteE, 0, noteC, 0, noteG,
            noteA, noteG, noteF, noteE, 0, noteC, 0, noteC, 0, noteDownG, 0,
            noteC, 0, 0, 0, noteC, 0, noteDownG, 0, noteC, 0, 0, 0, 0, 0, 0).notes;

    /** Humoresque melody */
    protected static ArrayList<Note> mytune2 = new Chord(0, 0, 0, 0, noteC,
            noteD, noteE, noteF, noteG, 0, noteG, 0, noteA, 0, noteA, 0, noteG,
            0, 0, 0, noteA, 0, noteA, 0, noteG, 0, 0, 0, noteF, noteF, noteF,
            noteF, noteE, 0, noteE, 0, noteD, 0, noteD, 0, noteG, 0, 0, 0,
            noteF, noteF, noteF, noteF, noteE, 0, noteE, 0, noteD, 0, noteD, 0,
            noteC, 0, 0, 0, 0, 0, 0).notes;

    /** 'Mary had a little lamb' melody */
    protected static ArrayList<Note> mytune3 = new Chord(0, 0, 0, 0, noteE, 0,
            noteD, 0, noteC, 0, noteD, 0, noteE, 0, noteE, 0, noteE, 0, 0, 0,
            noteD, 0, noteD, 0, noteD, 0, 0, 0, noteE, 0, noteG, 0, noteG, 0,
            0, 0, noteE, 0, noteD, 0, noteC, 0, noteD, 0, noteE, 0, noteE, 0,
            noteE, 0, noteE, 0, noteD, 0, noteD, 0, noteE, 0, noteD, 0, noteC,
            0, 0, 0).notes;
    ...
    /**
     * A constructor that initializes the piano roll with the tune of 'Mary had
     * a little lamb'
     */
    public PianoRoll() {
    ...
    /**
     * <p>
     * Produce the image of the piano roll notes on a staff.
     * </p>
     * <p>
     * Include instructions for playing:
     * </p>
     * <p>
     * Arrow keys play, pause, stop, and reverse
     * </p>
     * <p>
     * Three number keys choose the melody
     * </p>
     */
    public WorldImage pianoRollImage() {
        ...
        base = base
                .overlayImages(
                        new TextImage(new Posn(x + 10 + 40, y + 26),
                                "The Piano Roll: ", Color.red),
                        new TextImage(new Posn(x + 10 + 170, y + 26),
                                "Select tune 1, 2, or 3 ", Color.blue),
                        new TextImage(new Posn(x + 150 + 180, y + 26),
                                "Current tune: " + this.tune, Color.red),

                        // The instructions for playing
                        new TextImage(
                                new Posn(x + 30 + 170, y + 63),
                                "up: play    down: play reverse    left: stop    right: pause",
                                Color.blue),
                        new TextImage(
                                new Posn(x + 20 + 170, y + 44),
    ...
    /**
     * <p>
     * Draw there lines of staff on the given <code>Canvas</code>
     * </p>
     * 
     * <p>
     * Top line offset from the nw corner: 60
     * </p>
     * <p>
     * One note takes up 8 pixels square (disk of radius 4)
     * </p>
     * <p>
     * Next line of staff is 55 pixels below the previous one
     * </p>
     * 
     * @param c
     *            The <code>Canvas</code> where to draw
     */
    /*
     * protected void drawStaff(Canvas c){ // three rows of staff for (int row =
     * 0; row < 3; row++){
     * 
     * // find the base line for this staff int yline = y + 80 + row * 55; //
     * draw the five lines for (int i = 0; i < 5; i++) c.drawLine(new Posn(x,
     * yline + 8 * i), new Posn(x + width, yline + 8 * i), Color.black); } }
     */

    /**
     * <p>
     * Produce there lines of staff as an image
     * </p>
     * 
     * <p>
     * Top line offset from the nw corner: 60
    ...
    protected WorldImage staffImage(WorldImage base) {
        ...
        for (int row = 0; row < 3; row++) {
            ...
            // draw the five lines
            for (int i = 0; i < 5; i++)
                base = base.overlayImages(new LineImage(new Posn(x, yline + 8
                        * i), new Posn(x + width, yline + 8 * i), Color.black));
        }
        return base;
    }
    ...
    public RollChord nextChord() {
        if (this.next != 0) {
            ...
            this.current = (this.current + this.next + this.melody.size())
                    % this.melody.size();
    ...
    /**
     * record the state change: the piano roll should stop playing and reset to
    ...
    /**
     * <p>
     * Record the state change: the piano roll should pause at the current note
     * and stop playing.
     * </p>
     * <p>
     * To restart at the current place the user should select either 'play' or
     * 'playReverse' --- the current direction of playing is not retained.
     * </p>
     */
...
class RollChord extends Chord {
    ...
    protected static ArrayList<Integer> pitchlist = new ArrayList<Integer>(
            Arrays.asList(60, 62, 64, 65, 67, 69, 71, 72, 72, 74, 76, 77, 79,
                    81, 83, 84));
     ...
     * 
     * @param snotes
     *            the <code>String<code> that represent the notes 
     ...
     * 
     * @param pitches
     *            the pitches for the notes that comprise this chord
     */
     ...
     * 
     * @param notes
     *            the notes that comprise this chord
     */
    ...
    /**
     * draw this chord on the given <code>Canvas</code> at its place in the
     * piano roll given by the <code>index</code>
     * 
     * @param c
     *            the given <code>Canvas</code>
     * @param index
     *            the given location of this chord in the piano roll
     */
    /*
     * public void drawRollChord(Canvas c, int index){ for (Note note:
     * this.notes){ this.drawNote(c, note.pitch, index); } }
     */
     ...
     * <p>
     * <ul>
     * <li>There are 50 notes per line of staff for a total width of 400</li>
     ...
     * <li>2 the additional offset for the center of the circle of radius 4</li>
     * </ul>
     * </p>
     * 
     * <p>
     * Nothing is drawn if the pitch is other than chosen
     * </p>
     * 
     * @param c
     *            the <code>Canvas</code> where to draw
     * @param pitch
     *            the pitch of this note (should be one of the available ones)
     * @param index
     *            the position of this chord in the melody
     */
