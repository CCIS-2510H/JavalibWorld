...
/**
 * A comprehensive demo of the javalib.soundworld and javalib.tunes package: The
 * board that allows the programmer to choose the current instrument.
 * 
...
public class InstrumentBoard implements SoundConstants {
    ...
    protected static ArrayList<String> instrumentNames = new ArrayList<String>(
            Arrays.asList("PIANO", "ORGAN", "TIMPANI", "VIOLIN", "CLARINET",
                    "STEELDRUM", "CHOIR", "TUBA", "SAX", "PERCUSSION",
                    "WOOD_BLOCK", "BAGPIPE", "BIRD_TWEET", "SEASHORE",
                    "APPLAUSE", "BRASS SECTION"));

    /** the list of alternative instrument names to display */
    protected static ArrayList<String> instrumentAltNames = new ArrayList<String>(
            Arrays.asList("GUITAR", "PICOLO", "OCARINA", "TRUMPET",
                    "HARPSICHORD", "MARIMBA", "ACCORDION", "HARP", "OBOE",
                    "PERCUSSION", "HELICOPTER", "WHISTLE", "GOBLINS", "BANJO",
                    "STRINGS", "SITAR"));

    protected int[] altInstruments = new int[] { AcousticGuitar_nylon, Piccolo,
            Ocarina, Trumpet, Harpsichord, Marimba, Accordion, OrchestralHarp,
            Oboe, PERCUSSION, Helicopter, Whistle, FX6_goblins, Banjo,
            StringEnsemble2, Sitar };

    /** the list of colors to use for instrument choice boxes */
    protected static ArrayList<Color> colors = new ArrayList<Color>(
            Arrays.asList(Color.red, Color.blue, Color.yellow, Color.green,
                    Color.cyan, Color.orange, Color.magenta, new Color(25, 200,
                            100), new Color(200, 50, 50), Color.pink,
    ...
    protected WorldImage instrumentBoardImage() {
        ...
        WorldImage baseImage = new RectangleImage(center, 140, 400,
                Color.lightGray);
        ...
        // second line 15 below that
                new TextImage(this.move(nw, 80, 20 - 6), "Instrument Choice",
                        Color.red), new TextImage(this.move(nw, 80, 35 - 6),
                        "via mouse click", Color.red));

        // The frame for the currently selected instrument choice
        baseImage = baseImage.overlayImages(new RectangleImage(this.move(
                this.getCenter(this.current), -1, -1), size + 1, size + 1,
                Color.black));
        ...
        if (this.programSwitch) {
            ...
            for (int i = 0; i < 16; i = i + 1) {
                baseImage = baseImage.overlayImages(
                        new RectangleImage(this.getCenter(i), size - 2,
                                size - 2, colors.get(i)),
                        new TextImage(this.getStringCenter(i), instrumentNames
                                .get(i), Color.blue));
            }
        } else {
            // the alternate choice boxes and instrument names for all 16
            // instruments
            for (int i = 0; i < 16; i = i + 1) {
                baseImage = baseImage.overlayImages(
                        new RectangleImage(this.getCenter(i), size - 2,
                                size - 2, colors.get(i)),
                        new TextImage(this.getStringCenter(i),
                                instrumentAltNames.get(i), Color.blue));
            }
        }

        // add the program change button to the display
        baseImage = baseImage.overlayImages(new DiskImage(this.getCenter(16),
                size / 2 - 1, Color.black),
                new TextImage(this.getStringCenter(16), "PRGM CHANGE",
                        Color.black));
     ...
     *            the given position
     * @param dx
     *            the desired change in the horizontal direction
     * @param dy
     *            the desired change in the vertical direction
    ...
    /**
     * Get the position of the nw corner for the choice box for the given
     * instrument choice
     * 
     * @param index
     *            the given instrument choice
    ...
    /**
     * Get the position of the center corner for the choice box for the given
     * instrument choice
     * 
     * @param index
     *            the given instrument choice
    ...
    protected Posn getCenter(int index) {
        return new Posn(nw.x + xInset + (size - xInset) / 2, nw.y + yInset
                + index * (size));// + size / 2);
    }
    ...
    protected Posn getStringPos(int index) {
        return new Posn(nw.x + xStringInset, nw.y + yStringInset + index
                * (size));
    }
    ...
    protected Posn getStringCenter(int index) {
        return new Posn(nw.x + xStringInset + (140 - xStringInset) / 2, nw.y
                + yStringInset + index * (size) - size / 2 - 4);
    }

    /**
     * Determine which instrument has been selected by a mouse click. Produce
     * the given instrument if mouse click does not indicate an instrument
     * selection.
     * 
     * @param mousepos
     *            the mouse position when it is clicked
    ...
    public int selectInstrument(Posn mousepos, int currentInstrument) {
        ...
        if (mousepos.x < nw.x + xInset || mousepos.x > (nw.x + xInset + size))
