...
class Balloon {
    ...
    Balloon(Posn loc, int rad, IColor col, String s) {
        ...
        this.col = col;
        this.s = s;
        this.balloonImage = new DiskImage(this.loc, this.rad, this.col)
                .overlayImages(new LineImage(this.loc, new Posn(this.loc.x,
                        this.loc.y + 2 * this.rad), this.col), new TextImage(
                        new Posn(this.loc.x, this.loc.y + this.rad / 2), s,
                        new Black()));
    }

    // EFFECT:
    // draw this balloon on the given AppletAppletCanvas
    void drawBalloon(AppletCanvas c) {
        c.drawImage(new DiskImage(this.loc, this.rad, this.col));
        c.drawImage(new LineImage(this.loc, new Posn(this.loc.x, this.loc.y + 2
                * this.rad), this.col));
...
interface ILoB {
    ...
    public void draw(AppletCanvas c);
...
class SoundWorld extends World {
    ...
    new Integer[] { noteG, noteG, noteA, noteG, noteUpC, noteB, noteG, noteG,
            noteA, noteG, noteUpD, noteUpC, noteG, noteG, noteUpG, noteUpE,
            noteUpC, noteB, noteA, noteUpF, noteUpF, noteUpE, noteUpC, noteUpD,
            noteUpC };

    // The message to display on the balloons
    String[] hbm = new String[] { "Hap", "py", "Birth", "day", "to", "you",
            "Hap", "py", "Birth", "day", "to", "you", "Hap", "py", "Birth",
            "day", "Dear", "Matt", "hias", "Hap", "py", "Birth", "day", "to",
            "you" };

    // The locations of the centers of the balloons
    Posn[] posnsOld = new Posn[] { new Posn(25, 25), new Posn(75, 25),
            new Posn(125, 25), new Posn(175, 25), new Posn(50, 50),
            new Posn(150, 50), new Posn(25, 75), new Posn(75, 75),
            new Posn(125, 75), new Posn(175, 75), new Posn(50, 100),
            new Posn(150, 100), new Posn(25, 125), new Posn(75, 125),
            new Posn(125, 125), new Posn(175, 125), new Posn(50, 150),
            new Posn(100, 150), new Posn(150, 150), new Posn(25, 175),
            new Posn(55, 175), new Posn(85, 175), new Posn(125, 175),
            new Posn(155, 175), new Posn(185, 175) };

    // The locations of the centers of the balloons
    Posn[] posns = new Posn[] { new Posn(50, 50), new Posn(150, 50),
            new Posn(250, 50), new Posn(350, 50), new Posn(100, 100),
            new Posn(300, 100), new Posn(50, 150), new Posn(150, 150),
            new Posn(250, 150), new Posn(350, 150), new Posn(100, 200),
            new Posn(300, 200), new Posn(50, 250), new Posn(150, 250),
            new Posn(250, 250), new Posn(350, 250), new Posn(100, 300),
            new Posn(200, 300), new Posn(300, 300), new Posn(50, 350),
            new Posn(110, 350), new Posn(170, 350), new Posn(250, 350),
            new Posn(310, 350), new Posn(370, 350) };

    // The colors of the balloons
    IColor[] colors = new IColor[] { new Red(), new Green(), new Blue(),
            new Yellow(), new Red(), new Green(), new Blue(), new Yellow(),
            new Red(), new Green(), new Blue(), new Yellow(), new Red(),
            new Green(), new Blue(), new Yellow(), new Red(), new Green(),
            new Blue(), new Blue(), new Yellow(), new Green(), new Red(),
            new Yellow(), new Green() };

    // The radii of the balloons (if the text is longer, the balloon radius
    // increases)
    Integer[] radsold = new Integer[] { 20, 20, 23, 20, 20, 20, 20, 20, 23, 20,
            20, 20, 20, 20, 23, 20, 25, 25, 25, 20, 20, 23, 20, 20, 20 };

    // The radii of the balloons (if the text is longer, the balloon radius
    // increases)
    Integer[] rads = new Integer[] { 40, 40, 43, 40, 40, 40, 40, 40, 43, 40,
            40, 40, 40, 40, 43, 40, 50, 50, 50, 40, 40, 43, 40, 40, 40 };
    ...
    public void onTick() {
        ...
        if (this.tick == 0)
            this.bloons = new ConsLoB(new Balloon(posns[this.tick],
                    rads[this.tick], colors[this.tick], hbm[this.tick]),
                    new MtLoB());
        else
            this.bloons = new ConsLoB(new Balloon(posns[this.tick],
                    rads[this.tick], colors[this.tick], hbm[this.tick]),
    ...
    public static void main(String[] argv) {
        SoundWorld sw = new SoundWorld(0, new MtLoB());
...
class SoundWorldExamples implements SoundConstants {
    ...
    // sample lists of balloons
    ILoB mtlob = new MtLoB();
    ILoB threeBloons = new ConsLoB(this.blueBloon, new ConsLoB(this.redBloon,
            new ConsLoB(this.greenBloon, this.mtlob)));

    // a canvas for a visual test for drawing balloons
    AppletCanvas c1 = new AppletCanvas(100, 200);
    ...
    AppletCanvas c2 = new AppletCanvas(100, 200);
    ...
    void testOnTick(Tester t) {
        ...
        t.checkExpect(initWorld.bloons, new ConsLoB(new Balloon(
                new Posn(25, 25), 20, new Red(), "Hap"), this.mtlob));
        ...
        t.checkExpect(initWorld.bloons, new ConsLoB(new Balloon(
                new Posn(75, 25), 20, new Green(), "py"),
                new ConsLoB(
                        new Balloon(new Posn(25, 25), 20, new Red(), "Hap"),
                        this.mtlob)));

        // check the tune played: the tune bucket is emptied only on tick event,
        // not
        // when the onTick method is invoked -- so after two ticks the bucket
        // has
        // two tunes (here they are the same)
        t.checkExpect(initWorld.tickTunes.bucketSize(), 2);
        t.checkExpect(initWorld.tickTunes.contains(ORGAN, noteG), true,
                "organ note G");
        t.checkExpect(initWorld.keyTunes.bucketSize(), 0);
        ...
        t.checkExpect(initWorld.bloons, new ConsLoB(new Balloon(new Posn(125,
                25), 23, new Blue(), "Birth"), new ConsLoB(new Balloon(
                new Posn(75, 25), 20, new Green(), "py"),
                new ConsLoB(
                        new Balloon(new Posn(25, 25), 20, new Red(), "Hap"),
                        this.mtlob))));
        ...
        t.checkExpect(initWorld.tickTunes.contains(ORGAN, noteG), true,
                "organ note G");
        t.checkExpect(initWorld.tickTunes.contains(ORGAN, noteA), true,
                "organ note A");
        t.checkExpect(initWorld.keyTunes.bucketSize(), 0);
        ...
        t.checkExpect(initWorld.bloons, new ConsLoB(new Balloon(new Posn(185,
                175), 20, new Green(), "you"), this.threeBloons));
        t.checkExpect(initWorld.tickTunes.bucketSize(), 2);
        t.checkExpect(initWorld.tickTunes.contains(ORGAN, noteUpC), true,
                "organ note");
        t.checkExpect(initWorld.tickTunes.contains(WOOD_BLOCK, noteC), true,
                "wood block note");
        t.checkExpect(initWorld.keyTunes.bucketSize(), 0);
    ...
    void testOnKeyEvent(Tester t) {
        ...
        t.checkExpect(initWorld.keyTunes.contains(APPLAUSE, noteC));
        ...
        t.checkExpect(initWorld.tick, 3, "tick count check");
        t.checkExpect(initWorld.bloons, this.threeBloons);
        t.checkExpect(initWorld.keyTunes.bucketSize(), 2);
        t.checkExpect(initWorld.keyTunes.contains(APPLAUSE, noteC));
        t.checkExpect(initWorld.keyTunes.contains(BIRD_TWEET, noteUpC));
        t.checkExpect(initWorld.tickTunes.bucketSize(), 0);
